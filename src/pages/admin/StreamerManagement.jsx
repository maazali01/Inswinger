...
import { MdVerified, MdPending, MdEmail, MdCalendarToday, MdClose, MdCheckCircle, MdCancel, MdImage } from 'react-icons/md';
...
export default function StreamerManagement() {
  ...
  const fetchStreamers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('users')
        .select('id, name, email, is_verified, plan, screenshot_url, created_at')
        .eq('role', 'streamer')
      ...
      toast.error(err.message || 'Failed to load streamers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReject = async (id) => {
    if (!confirm('Reject this streamer verification? This will keep them unverified.')) return;
    try {
      setRejectingId(id);
      const { error } = await supabase.from('users').update({ is_verified: false }).eq('id', id);
      if (error) throw error;
  ...
  return (
    <>
      <Helmet>
        <title>Streamer Management | Admin - Inswinger+</title>
        <meta name="description" content="Manage streamers, verify accounts, and review payment screenshots" />
      ...
      <div className="min-h-screen bg-gray-900 text-gray-100">
        ...
        <main className="flex-1 p-8 md:ml-64">
          <header className="mb-8">
            ...
            <p className="mt-2 text-gray-400">Approve streamers and review payment screenshots</p>
          </header>
              ...
              filteredStreamers.map(s => (
                ...
                >
                  <div className="flex flex-col gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {s.plan && (
                            <p className="text-xs text-gray-500 mt-1">
                              Plan: <span className="text-blue-400">{s.plan}</span>
                            </p>
                          )}
                    ...
                    {/* Screenshot Section - Button Only */}
                    {s.screenshot_url && (
                      <div className="pt-4 border-t border-gray-700">
                        <button
                          onClick={() => setSelectedScreenshot({ url: s.screenshot_url, name: s.name })}
                          className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                        >
                          <MdImage className="text-xl" />
                          <span className="font-medium">View Payment Screenshot</span>
                        </button>
                      </div>
                    )}
                  </div>
          ...
          <ImageViewerModal
            ...
            title={`Payment Screenshot - ${selectedScreenshot?.name || 'Streamer'}`}
          />
        </main>
      </div>
    </>
